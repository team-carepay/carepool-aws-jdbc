...
class RdsIamPasswordPluginTest {
  ...
  void setUp() {
    plugin =
        new RdsIamPasswordPlugin(
            () -> new Credentials("ABC", "DEF", "GHI"), () -> "eu-west-1", clock);
    //noinspection unchecked
    protocol = mock(Protocol.class);
    propertySet = new DefaultPropertySet();
    when(protocol.getPropertySet()).thenReturn(propertySet);
    when(protocol.getSocketConnection())
        .thenReturn(
            new FakeSocketConnection(
                "cp-ke-test-fake-db.cluster-c3u8jposmdov.eu-west-1.rds.amazonaws.com", 3306));
  }

  @Test
  void testInit() {
    plugin.init(protocol, null);
    assertThat(propertySet.getEnumProperty(PropertyKey.sslMode).getValue())
        .isEqualTo(PropertyDefinitions.SslMode.VERIFY_CA);
    assertThat(propertySet.getStringProperty(PropertyKey.trustCertificateKeyStoreType).getValue())
        .isEqualTo("PEM");
  }
